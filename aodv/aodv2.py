...
from adhoccomputing.GenericModel import GenericModel, Event, GenericMessage, GenericMessageHeader, GenericMessagePayload, EventTypes, Thread, Topology, ConnectorTypes, MessageDestinationIdentifiers
from .Broadcasting import BroadcastingMessageHeader
...
class AODVLayer(GenericModel):
    ...
    def on_rreq(self, eventobj: Event):
        ...
        broadcastmessage = AODVBroadcastingMessage(
            header, applicationLayerMessage)
    ...
    def on_message_from_top(self, eventobj: Event):
        ...
        if (nexthop == inf):
            ...
            evt = Event(self, AODVLayerMessageType.RREQ,
        ...
        if broadcastingMessageHeader.messagetype == AODVLayerMessageType.RREQ:
            ...
            with self.rreqLock:
                if (applicationLayerMessage.uniqueid in self.broadcastdb):
                    print(
                        f"RREQ Message Received Already # Receiver: {self.componentname}.{self.componentinstancenumber} from node: {previousNode} with uniqueid {applicationLayerMessage.uniqueid}. Skipping...")
                    ...
                    if (source == self.componentinstancenumber):
                        print(
                            f"RREQ Message Arrived to Source # Receiver: {self.componentname}.{self.componentinstancenumber} from node: {previousNode} with uniqueid {applicationLayerMessage.uniqueid}. Skipping...")

                    else:
                        print(
                            f"RREQ Message Received # Receiver: {self.componentname}.{self.componentinstancenumber} from node:{previousNode} for source:{source}")
                        self.update_routing_table(
                            source, previousNode, hopCount, sequencenumber)
                        ...
                        if destination == self.componentinstancenumber:
                            ...
                            print(
                                f"RREP Message Send # Sender: {self.componentname}.{self.componentinstancenumber} for source {source} for destionation {destination}")
                            evt = Event(self, AODVLayerMessageType.RREP,
                                        applicationLayerMessage)
                            self.send_self(evt)
                            ...
                            time.sleep(random.randint(1, 1))
                            print(
                                f"Node Knows Destination # Receiver: {self.componentname}.{self.componentinstancenumber} for source {source} for destionation {destination}")
                            print(
                                f"RREP Message Send # Sender: {self.componentname}.{self.componentinstancenumber} for source {source} for destionation {destination}")
                            evt = Event(self, AODVLayerMessageType.RREP,
                                        applicationLayerMessage)
                            ...
                            time.sleep(random.randint(1, 1))
                            print(
                                f"RREQ Message Resend # Sender: {self.componentname}.{self.componentinstancenumber} for source {source} for destination {destination}")
                            evt = Event(self, AODVLayerMessageType.RREQ,
                                        applicationLayerMessage)
                            self.send_self(evt)
            ...
            with self.rrepLock:
                if (applicationLayerMessage.uniqueid in self.rrepdb) and (self.rrepdb[applicationLayerMessage.uniqueid]["hopCount"] <= hopCount):
                    print(
                        f"RREP Message Received Already # Receiver: {self.componentname}.{self.componentinstancenumber} from node: {previousNode} with uniqueid {applicationLayerMessage.uniqueid}. Skipping...")

                else:
                    self.rrepdb[applicationLayerMessage.uniqueid] = {
                        "hopCount": hopCount}
                    self.update_routing_table(
                        destination, previousNode, hopCount, sequencenumber)

                    if (source == self.componentinstancenumber):
                        print(
                            f"RREP Message Arrived to Source # Receiver: {self.componentname}.{self.componentinstancenumber} from {previousNode} with uniqueid {applicationLayerMessage.uniqueid}")
                        print(
                            f"DATA Message Send # Sender: {self.componentname}.{self.componentinstancenumber} for source {source} for destionation {destination}")
                        evt = Event(self, AODVLayerMessageType.DATA,
                                    applicationLayerMessage)
                        self.send_self(evt)

                    else:
                        print(
                            f"Received RREP Message # Receiver: {self.componentname}.{self.componentinstancenumber} from {previousNode} for source {source}")
            ...
            if (destination == self.componentinstancenumber):
                print(
                    f"DATA Message Arrived to Destination # Receiver: {self.componentname}.{self.componentinstancenumber} from {previousNode} with uniqueid {applicationLayerMessage.uniqueid}")
                self.send_up(
                    Event(self, EventTypes.MFRB, eventobj.eventcontent))

            else:
                print(
                    f"Received DATA Message # Receiver: {self.componentname}.{self.componentinstancenumber} from {previousNode} for destination {destination}")
    ...
    def get_next_hop(self, destination, destinationSequenceNumber):
        retval = self._get_next_hop_data(
            destination, destinationSequenceNumber)
        if retval != inf:
    ...
    def update_routing_table(self, destination, nextHop, hopCount, destinationSequenceNumber):
        ...
        if ((retval == inf) or (hopCount < retval["hopCount"])):
            print(
                f"Routing Table Updated # Node: {self.componentname}.{self.componentinstancenumber} Data => destination:{destination} nextHop:{nextHop} hopCount:{hopCount} sequenceNumber:{destinationSequenceNumber}")
